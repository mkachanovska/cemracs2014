...
\mrev{Importantly, since $\delta(x)=0$, in this case no resonance occurs and $E_x(x)=0$. This corresponds to the case when the
tensor of dielectric permittivity $\underline{\varepsilon}^{\nu}$ is 
diagonal.} 
...
\end{tabular}
\caption{Dependence of the solution $E_{y}^{\nu}(x_c,t)$ \mrev{to the problem described in Section \ref{section:absorption_airy}} on time for large times is demonstrated in the left figure.  
In the right figure we show the solution \mrev{to the same problem} for $\nu=10^{-2}$, for two fixed moment of times.}
...
\end{tabular}
\caption{In the left figure we plot the dependence of the solution $E_{y}^{\nu}(x_c,t)$ \mrev{to the problem described in Section \ref{section:absorption_airy}} on time $t$, with $\nu=10^{-4}$ and $x_c=0.45$. 
...
\includegraphics[width=0.45\textwidth]{pics_time_domain/res/ey_fixed_x_1e3-crop.pdf}
 \end{tabular}
\caption{The figures demonstrate the dependence of the solutions to the problem \mrev{described in Section \ref{sec:resn}}
...
\begin{figure}
 ...
 the problem \mrev{described in Section \ref{sec:resn}} on time $t$ for fixed 
...
$E_x^{\nu}(x,t)$ to the problem \mrev{described in Section \ref{sec:resn}} on $x$ for fixed values of $t$. }
...
$E_y^{\nu}(x,y)$ to the problem \mrev{described in Section \ref{sec:resn}} on $x$ for fixed values of $t$. 
...
\caption{The figures demonstrate the dependence of the solutions 
$E_x^{\nu}(x,t)$ and $E_y^{\nu}(x,y)$ to the problem \mrev{described in Section \ref{sec:resn}} on $x$ for fixed values of $t$. While the solution in the time-domain 
