...
the cyclotron frequency $w_c(\mathbf{r})=\frac{e B_0(\mathbf{r})}{m_e}$ and rewrite \ref{eq:main_system} in the frequency domain ($\partial_t\rightarrow iw$), for a 
particular case $\nu=0$:
\begin{align*}
...
\begin{align*}
 ...
 \end{matrix}
\right),\; \alpha(\mathbf{r})=1-\frac{w_p^2}{w^2-w_c^2},\; \delta=\frac{w_cw_p^2}{w(w^2-w_c^2)}.
\end{align*}
