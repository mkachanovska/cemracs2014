...
\end{split}
\end{align}
coupled with an equation for the linear electronic current  $\J = eN_e(\mathbf{r}) \ubf$, 
\begin{align}
\begin{split}
...
\end{cases}
\end{equation}
 posed in $(-L,\; H)$, for some $L,H>0$, with \mrev{the (simplified) boundary conditions compatible with (\ref{eq:boundary_conditions_intro})}
 \begin{align}
...
a simple root in $x_h$ and has no other roots in an $\epsilon$-vicinity of $x_h$ $B_{\epsilon}(x_h)$, for some $\epsilon>0$, and 
$\delta(x_h)\neq 0$. 
\end{df}
}
