...
and
\ben
\frac{d}{dt}{\mathcal E}=-\nu\left(\|u_{x}\|_{L_{2}(\Omega)}^2+\|u_{y}\|_{L_{2}(\Omega)}^2\right)+\text{boundary terms}.
\een
...
The point $\mathbf{r}_c=(x_c,y_c)$ s.t. $\omega_c(\mathbf{r}_c)=\omega$, i.e. when $\underline{\varepsilon}_{\omega}^{0}(\mathbf{r})$ is singular, is called a cyclotron resonance (c.f. \cite[Chapter 1-5]{Stix}). 
...
Another interesting case is $\omega_p^2(\mathbf{r})+\omega_c^2(\mathbf{r})=\omega^2$, when the diagonal part of the tensor $\underline{\varepsilon}_{\omega}^{0}(\mathbf{r})$ vanishes, 
...
\label{eq:seq_regularized}
  \mathbf{curl}{\operatorname{curl}}\hat{\mathbf{E}}-\frac{\omega^2}{c^2}\underline{\varepsilon}^{\nu}_{\omega}(\mathbf{r})\hat{\mathbf{E}}=0,\;
  \underline{\varepsilon}^{\nu}_{\omega}(\mathbf{r})= \underline{\varepsilon}^{0}_{\omega}(\mathbf{r})+i\nu \mathbb{E},\; 
\end{align}
with $\nu\rightarrow 0$ and $\mathbb{E}$ being an identity matrix. 
}


\urev{We will concentrate on a simplified model (which nevertheless is of physical interest when modelling antennas in plasmas), where it is required to study the above system in an 
infinite domain $(-L,\; \infty)\times\mathbb{R}$, $L\in \mathbb{R}$, under the assumption that the electron density $N_{e}$ and the magnetic field $B_0$ are \textbf{uniform} in the second coordinate, more precisely 
$N_{e}=N_{e}(x)$ and $B_0=B_0(x)$, see e.g. \cite{Despres_2014}. Despite such a simplfication, already a 1D-version of the system (\ref{eq:main_model}) contains difficulties intrinsic to the hybrid resonance case
\cite{Despres_2014, singular_solutions}. 

The problem (\ref{eq:model_freq_domain}) should be equipped with the boundary conditions.
...
$(-L,\; \infty)\times\mathbb{R}$ to $(-L,\; H)\times\mathbb{R}$ and set on the right boundary $\operatorname{curl} \E = 0$. 

}

Hence, recalling that $N_e=N_e(x), \; B_0=B_0(x)$, we perform the Fourier transform in $y$ ($\partial_y\rightarrow i\theta, \; \theta\in\mathbb{R},$) and consider the one-dimensional setting
...
\end{equation}
 ...
 \end{align*}
This is the model that will be considered in the rest of the article, unless stated otherwise. 
\urev{
Indeed, the dielectric tensor for this problem with $\nu=0$ and its coefficients $\alpha_{\omega},\;\delta_{\omega}$, 
see \ref{eq:alpha_delta}, depend solely on $x$, $\omega$ and $\theta$.  

We concentrate on the physical situation of the hybrid resonance, which we define as in \cite{singular_solutions}.
\begin{df}
Let $\omega,\theta\in\mathbb{R}$ be fixed. 
We will call a point $x_h\in \mathbb{R}$ a hybrid resonance if $\alpha_{\omega}$ has 
a simple root in $x_h$ and has no other roots in an $\epsilon$-vicinity of $x_h$ $B_{\epsilon}(x_h)$, for some $\epsilon>0$. 
\end{df}
}
