...
\end{align}
\mrev{We additionally assume that $E_y^{\nu}(x)$ is sufficiently smooth and $\|(E_y^{\nu})''\|_{H_{2}(\Omega)}$
...
\end{itemize}
\mrev{The estimate (\ref{eq:cea}) combined with (\ref{eq:interp}) results in, for some $C_1,\;C_2, \; C>0$:
\begin{align}
  \label{eq:final_est}
...
\een
From (\ref{eq:final_est}) it thus follows that to ensure (\ref{eq:problem1}) $h$ should be chosen as 
\begin{align}
\label{eq:estimate_h}
 h=\alpha_{\epsilon}\nu^{\frac{7}{4}},
\end{align}
