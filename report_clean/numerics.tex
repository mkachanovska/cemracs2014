...
\label{eq:eynuprime}
 \left(\tilde{E}_{y}^{\nu}\right)'&=\frac{-x+i\nu}{\rho_{\nu}(x-X_{\nu})}p_{\nu}(x)-\log\rho_{\nu}(x-X_{\nu})p_{\nu}(x)+p'_{\nu}(x)(-x+i\nu)\log\rho_{\nu}(x-X_{\nu}).
\end{align}
The first term in the above expression is well-defined in $x=0$, even for $\nu=0$ (the limit can be obtained with the help of the L'Hopital's rule 
and recalling that $\rho_{\nu}=O(x-X_{\nu})$ in the vicinity of the resonance), and is uniformly bounded in $\nu$, together with its derivatives. 
...
\begin{align*}
 \left(\log\rho_{\nu}(x-X_{\nu})p_{\nu}\right)'=\frac{p_{\nu}}{\rho_{\nu}(x-X_{\nu})}\rho'_{\nu}(x-X_{\nu})+\log\rho_{\nu}(x-X_{\nu})p_{\nu}'.
\end{align*}
The most singular term in the above expression is $\frac{p_{\nu}}{\rho_{\nu}(x-X_{\nu})}\rho'_{\nu}(x-X_{\nu})$ which can be bounded, for all sufficiently small $x$, by 
\begin{align*}
 \left|\frac{p_{\nu}\rho'_{\nu}(x-X_{\nu})}{\rho_{\nu}(x-X_{\nu})}\right|\leq \frac{C}{|x-i\nu c|},\; C>0,
\end{align*}
where we used an explicit form of $\rho_{\nu}=G(x-X_{\nu})+O((x-X_{\nu})^2), G\in\mathbb{R},$ valid for sufficiently small $\nu$ and $|x|$, as well as 
the expansion $X_{\nu}=ic\nu+O(\nu^2)$. 
...
To do so, we conduct the  numerical experiment with parameters for the problem (\ref{eq:main_frequency_domain} as given in Table \ref{tab:parameters}. \mrev{Recall as well that we choose $\theta=0$. }
\begin{table}[htb!]
\begin{tabular}{l|l}
Parameter & Value \\
\hline
