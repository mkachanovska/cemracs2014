...
\end{align*}
One of the solutions of this equation, an Airy function $\Ai(x)$, see \cite[Chapter 10.4]{abramowitz_stegun}, is analytic and 
decays rapidly as $x\rightarrow \infty$. }
\mrev{Hence, choosing $H$ sufficiently large (the actual value used in the experiments $H=10$, where $\Ai'(10)\approx -3.5e-10$) and the boundary conditions as 
\begin{align}
\label{eq:bcs_airy}
...
\urev{Since the coefficients of the problem are smooth, we expect the solution to be at least in $C^2(\Omega)$.}  
\begin{figure}
\begin{tikzpicture}
    \begin{loglogaxis}[
        xlabel=$h$,
...
font=\small
    ...
    \end{loglogaxis}
    \end{tikzpicture}
    \caption{Convergence rates for \mrev{the problem (\ref{eq:main_frequency_domain}) discretized as (\ref{eq:simple_system})} 
...
\mrev{and $E^{\nu}_{x},\;E^{\nu,h}_{x}$ being correspondingly the exact solution and the solution computed with the help $\mathbf{V}_h$, }
...
\begin{align*}
E_{y}^{\nu}(x)=\tilde{E}_{y}^{\nu}(x)+\text{smooth terms with derivatives bounded independently of $\nu$,} \\
\tilde{E}_{y}^{\nu}(x)=\log\rho_{\nu}(x-X_{\nu})(\alpha(x)+i\nu)p_{\nu}(x),
\end{align*}
where $\rho_{\nu}$ is a smooth function s.t. $\rho_{\nu}(0)=0,\; \rho_{\nu}'(0)\neq 0$, the point $X_{\nu}\approx i\nu c+O(\nu^2)$, for some $c\in\mathbb{R}$, 
and $p_{\nu}(x)$ is a smooth function with derivatives bounded independently of $\nu$. 
}
...
\end{align}
The first term in the above expression is well-defined in $x=0$ (the L'Hopital's rule), and for sufficiently small $\nu$ can be bounded independently of $\nu$. 
Similarly, the third term is non-singular. To examine the behaviour of $\left(\tilde{E}_{y}^{\nu}\right)''$, we study solely the singular second term of (\ref{eq:eynuprime}) (the derivatives for the rest of the terms can be shown to be bounded 
independently of $\nu$)
\begin{align*}
 \left(\alpha'\log\rho_{\nu}(x-X_{\nu})p_{\nu}\right)'=\alpha''\log\rho_{\nu}(x-X_{\nu})p_{\nu}+\frac{\alpha'p_{\nu}}{\rho_{\nu}(x-X_{\nu})}\rho'_{\nu}(x-X_{\nu})+
 \alpha'\log\rho_{\nu}(x-X_{\nu})p_{\nu}'.
\end{align*}
The most singular term in the above expression is $\frac{\alpha'p_{\nu}}{\rho_{\nu}(x-X_{\nu})}\rho'_{\nu}(x-X_{\nu})$ which can be bounded, for sufficiently small $x$, 
\begin{align*}
 \left|\frac{\alpha'p_{\nu}}{\rho_{\nu}(x-X_{\nu})}\rho'_{\nu}(x-X_{\nu})\right|\leq \frac{C}{|x+i\nu c|},\; C>0,c\in\mathbb{R},
\end{align*}
where we used an explicit form for $X_{\nu}$. 
...
Denoting by \mrev{$E_{x}^{\nu,h}$ the solution $E_{x}(x)$} computed on the mesh with a \mrev{ width $h$ with 
the absorption $\nu$, and by $E_{x}^{\nu}$} the solution computed on a mesh multiple times finer (\mrev{more precisely, the reference mesh
...
In the right figure we show the computed solutions for this problem (\mrev{for brevity denoted as $E_{x,y}^{\nu}$}) for different values of $\nu$. }
