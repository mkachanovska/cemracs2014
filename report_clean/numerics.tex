...
\begin{align*}
E_{y}^{\nu}(x)=\tilde{E}_{y}^{\nu}(x)+\text{smooth terms, which, together with their derivatives, are bounded independently of $\nu$,} \\
\tilde{E}_{y}^{\nu}(x)=\log\rho_{\nu}(x-X_{\nu})(\alpha(x)+i\nu)p_{\nu}(x),
\end{align*}
where $\rho_{\nu}$ is a smooth function s.t. $\rho_{\nu}(0)=0,\; \rho_{\nu}'(0)\neq 0$, the point $X_{\nu}\approx i\nu c+O(\nu^2)$, for some $c\in\mathbb{R}$, 
and $p_{\nu}(x)$ is a smooth function, which, together with its derivatives,  is bounded independently of $\nu$. 
}
