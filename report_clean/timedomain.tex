...
\arev{
In this section we develop the discretization of the time-dependent one-dimensional model (\ref{eq:main_model}), for a particular case $\theta=0$. 
In the first part of this section we show how this system can be discretized with the help of 
a version of the Yee scheme suggested in \cite{stable_yee_plasma_current}, and in the second part a scheme based on semi-lagrangian discretization is employed.}
\subsection{A staggered scheme}
\label{sec:staggered_discretization}
In this section, we develop a discretization of the time-dependent problem~\eqref{eq:main_model} 
...
of \mrev{$E_x$}, $u_x$ (black dots), $E_y$, $u_y$ (blue dots) and $H_z$ (red dots) 
