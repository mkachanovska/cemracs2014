...
\frac{\omega^2}{c^2}\underline{\epsilon}^{\nu}_{\omega}(\mathbf{r})\hat{\mathbf{E}}=0,\\
...
\bealn
 \alpha^{\nu}_{\omega}=\alpha^{0}_{\omega}+i\nu\frac{\omega_p^2(\omega^2+\omega_c^2)}{(\omega^2-\omega_c^2)^2}=\alpha^{0}_{\omega}+i\nu D_{\alpha},\\
 \delta^{\nu}_{\omega}=\delta^{0}_{\omega}-i\nu\frac{2\omega}{(\omega^2-\omega_c^2)^2}=\alpha^{0}_{\omega}+i\nu D_{\delta},
\eealn
where $\alpha^{0}_{\omega},\;\delta^{0}_{\omega}$ are defined by (\ref{eq:alpha_delta}). In the frequency domain one thus can consider 
the following regularization of the cold plasma dielectric tensor of (\ref{eq:main_frequency_domain}):
\begin{align*}
\underline{\epsilon}^{\nu}_{\omega}\approx \underline{\epsilon}^0_{\omega}+i\nu \left(
 \begin{matrix}
  D_{\alpha} & D_{\delta}\\
  -D_{\delta} & D_{\alpha}
 \end{matrix}
...
\textcolor{red}{Thus, applying the above process to (\ref{eq:main_model}), we obtain, see also \cite{Despres_2014},
\begin{align}
...
\begin{matrix}
 \alpha_{\omega}^{0}(x) & i\delta_{\omega}^{0}(x) \\
 -i\delta_{\omega}^{0}(x) & \alpha_{\omega}^{0}(x)
\end{matrix}
\right)=
\left(
\begin{matrix}
...
\right).
\end{align}
 In this model problem, the coefficients  $\alpha(x)$ and $\delta(x)$ are sufficiently smooth, i.e. bounded and continuous in  
$\left[-L,\; H\right)$.
}
