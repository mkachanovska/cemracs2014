...
\begin{remark} \mrev{
The coercivity estimates in the above proof are not optimal in the case when $\alpha(x)$ is bounded away from zero on the whole interval $[-L,\; H]$, at least 
...
$\alpha \in C_{0}([-L,\;H])$ and is bounded away from zero on the whole interval), this equation is equivalent to 
\begin{align}
\label{eq:mult_eq}
...
had been demonstrated in \cite{LMIG_thesis}. Provided the solution $E_y$, we can obtain $E_x$ from the equation (\ref{eq:mult_eq}).}
\end{remark}
...
an explicit expression for the tensor of the dielectric permittivity (\ref{eq:epsilon_0}), we obtain the following formulation.
%\begin{align*}
%\begin{split}
...
For all $ \tilde E_x\in L_{2}(\Omega), \; \tilde{E}_{y}\in H_{1}(\Omega)$,
\begin{align}
\label{eq:var_form2}
\begin{split}
...
To discretize the problem (\ref{eq:var_form2}), we approximate $\mathbf{E}$ as 
\begin{align*}
