...
\[
 \left(E_x,\overline{\alpha} \tilde{E}_x\right)=-\left(i\delta E_y,  \tilde{E}_x\right).
\]
...
\begin{align*}
 \displaystyle \int_{-L}^H E_y'\tilde {E_y'} - \int_{-L}^H \left(\alpha-\frac{\delta^2}{\alpha}\right)E_y \overline{\tilde{E}}_{y}
  - \imath \lambda E_y (-L) \overline{\tilde E_y} (-L) &= -g_{inc} (-L) \overline{( \tilde E_y(-L) )},\\
  \text{ for all } \tilde{E}_{y}\in H_{1}(\Omega).
\end{align*}
