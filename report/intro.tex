...
\frac{\epsilon_0 |\E(t,\x)|^2}{2}+\frac{ |\B(t,\x)|^2}{2\mu_0}+\frac{m_e N_e|\mathbf{u} (t,\x)|^2}{2} 
\right)\mathrm{d}\x.
\een}
\mrev{An energy-preserving scheme for discretizing (\ref{eq:main_model}) was recently developed in \cite{Despres_2014}. 
It can be shown that for non-zero collision frequency $\nu$ the loss of energy is proportional to $\nu$, however, as $\nu\rightarrow0+$, it 
does not vanish. This behaviour had been recently investigated in \cite{Despres_2014}}.
...
$\lim\limits_{\nu\rightarrow 0}\lim\limits_{t\rightarrow+\infty}\mathbf{E}(t)$, i.e. we check the validity of the scheme in Fig.~\ref{fig:limits}. 
\begin{figure}
\begin{tikzpicture}
