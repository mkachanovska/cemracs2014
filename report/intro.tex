...
the Fourier transform in $y$. Denoting by $\theta$ a corresponding Fourier variable, the model \eqref{eq:main_model} can be rewritten as follows
\begin{align}
...
integrable, and explicit estimates on the behaviour of the solutions of \eqref{eq:main_model_theta} were given. 
...
problem \eqref{eq:main_model_theta} written in the frequency domain:
\begin{align*}
\end{align*}
...
the FDTD discretization \eqref{eq:main_model_theta}, suggested in \textcolor{red}{citationtofix}\cite{}. We compare this result with 
