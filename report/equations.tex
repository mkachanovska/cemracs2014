...
\end{align*}
is continuous and coercive: for all $\ubf,\vbf\in \mathbf{V}$ it holds that
\begin{align}
...
Given $E_x,\; E_y\in L_{2}(\Omega)$, the function $\mathcal{F}=\frac{i\delta}{(\alpha+\imath\nu)}\left((\alpha+\imath\nu)E_{x}+i\delta E_{y}\right)$ also belongs to $L_{2}(\Omega)$. 
...
\begin{align*}
\int_{-L}^{H}\left(i\delta E_{x}-\delta^2(\alpha+i\nu)^{-1} E_{y}\right)\overline{\tilde E}_{y}=0
\end{align*}
to obtain the variational formulation for $E_{y}$, which will coincide with the variational formulation for the Helmholtz equation, and thus will be well-posed 
(see the thesis \cite{thesis_lmig}).
The estimates from \cite{thesis_lmig} can be employed to bound $\|E_{x}\|_{L_{2}}$.
\end{remark}
