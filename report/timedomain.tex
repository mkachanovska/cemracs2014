...
EXPLAIN HOW IS THIS SYSTEM OBTAINED FROM THE SYSTEM PROVIDED IN THE INTRO
...
\end{array}\right.
 \end{align}
The space discretization is performed on staggered grid, meaning $E_x$, $u_x$, $E_y$, $u_y$ and $H_z$ are not and the same grid points. RAPID JUSTIFICATION ?  Fig.~\ref{schemepos} shows the positions of $Ex$, $u_x$ (black dots), $E_y$ $u_y$ (blue dots) and $H_z$ (red dots) on the discretized grid in space and time. The fully discretized system then reads
