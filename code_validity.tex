...
draw=none
    ...
    \addplot[mark=*,blue] table {E1L2.dat}; 
    \addlegendentry{$\|E_{1}-E_{1}^{c}\|_{L_{2}}$};
    \addplot[mark=*,red] table {E2H1.dat};
        \addlegendentry{$\|E_{2}-E_{2}^{c}\|_{H_{1}}$};
        
          \addplot[mark=*,cyan] table {E2L2.dat};
        \addlegendentry{$\|E_{2}-E_{2}^{c}\|_{L_{2}}$};  
        \addplot[dotted] table{E_h2.dat};
         \addlegendentry{$O(h^2)$};
         \addplot[dashed] table{H.dat};
         \addlegendentry{$O(h)$};
    \end{loglogaxis}
    \end{tikzpicture}
