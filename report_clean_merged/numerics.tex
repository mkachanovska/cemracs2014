...
performed with \urev{our} code written in Octave.  
...
\end{align}
\urev{In this case $E_{y}$ that solves (\ref{eq:main_frequency_domain})  satisfies the Airy equation, see also Remark \ref{remark:other},
\begin{align*}
 E_y''-xE_y=0,\qquad x\in [-L,\; H].
\end{align*}
The physical solution of this equation, the Airy function \urev{$\Ai(x)$}, see \cite[Chapter 10.4]{abramowitz_stegun}, is analytic and 
decays rapidly as $x\rightarrow \infty$. }
\mrev{Hence, choosing $H$ sufficiently large (the actual value used in the experiments $H=10$, where $\left|\Ai'(10)\right|<1e-9$) and the boundary conditions
\begin{align}
\label{eq:bcs_airy}
\partial_{x}E_{y}(-L)+2iE_{y}(-L)=2i\Ai(-L)+\Ai'(-L),\qquad 
\partial_{x}E_{y}(H)=0\approx \Ai'(H),
\end{align}
the exact solutions are: $\Ai(x)$ for the unknown $E_{y}$, and $i\frac{\delta(x)}{\alpha(x)}\Ai(x)$ for $E_{x}$. \urev{Hence both functions are smooth}. 
...
For simplicity, let us assume \mrev{that $\alpha(x)$ is smooth and }
\begin{align}
\label{eq:cond}
...
\item \mrev{An explicit bound on $\|\left(E_{x}^{\nu}\right)''\|_{L^{2}(\Omega)}$ from \cite{Despres_2014} and \cite[Proposition 1]{singular_solutions}. 
First of all, we 
...
the point $X_{\nu}\approx i\nu c+O(\nu^2)$, for some $c\in\mathbb{R}_{*}$. 
}
\item \mrev{An explicit bound on $\|\left(E_{y}^{\nu}\right)''\|_{L^{2}(\Omega)}$ from \cite{singular_solutions}. As before, we 
...
The same holds for the second term. To examine the behaviour of $\left(\tilde{E}_{y}^{\nu}\right)''$, 
...
(\ref{eq:main_frequency_domain}) as given in Table \ref{tab:parameters}. \mrev{Recall as well that $\theta=0$. }
\begin{table}[htb!]
\begin{tabular}{l|l}
Parameter & Value \\
\hline
...
\end{table}
Denoting by \mrev{$E_{x}^{\nu,h}$ the solution $E_{x}(x)$} computed on the mesh with \mrev{width $h$ and 
...
\eealn
 is ensured by choosing $h$ as \mrev{$\beta_{\epsilon}\nu^{\frac{3}{2}}$}, \mrev{$\beta_{\epsilon}>0$.}
\end{remark}
...
case.
The numerical tests have been kept to a minimum since \arev{much more} is needed to fully validate
the concept.
...
\urev{Notice that in this case we shifted the computational domain so that $L=0$ and the point where the resonance occurs is $x_h=20$.}
