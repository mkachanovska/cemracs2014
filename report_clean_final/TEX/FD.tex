...
\eealn
where the frequency 
$\omega_{\nu}=\omega+i\nu$ is shifted in the complex plane. \arev{ %Notice that neither cyclotron, nor hybrid resonances occur in the problem 
%with the dielectric tensor $\uuline{\varepsilon_{\omega}^{\nu}}(x), \; \nu>0$ (since all the problem parameters are real). Hence 
%t
...
\een
yields to the same limit solution as $\nu\rightarrow 0$. }
...
\mrev{In this section we study the well-posedness of Problem (\ref{eq:main_frequency_domain}) with the 
...
\label{vf1dcase}
\begin{array}{l}
\displaystyle \int_{-L}^H (E_y' -i\theta E_x)\overline{(\widetilde E_y' -\urev{i} \theta \widetilde E_x)} 
...
%=\omega\geq 0$. 
Let us remark that in this and further sections, where it is not ambiguous, we will use the notation $E_{x,y}$ instead of $\widehat{E}_{x,y}$ for convenience. \mrev{Also, 
...
For the sake of conciseness, consider the following definitions. Let $a$ and $l$ be the bilinear and linear forms, \urev{defined for $\mathbf{u},\; \mathbf{v}\in \mathbf{V}$}:
\ben
...
\end{array}\right.
\een
Then \eqref{vf1dcase} reads: \urev{find $\mathbf{E}$ such that},
\ben
...
\urev{Let Assumption \ref{assumption:smooth} hold}. Then the bilinear form 
\ben
...
\begin{proof}
	...
	On the other hand, \mrev{for $\lambda \geq 0$},
	$
	a_2 (\ubf ,\ubf) \leq -\nu \| \ubf \|_{L^2}^2$. 
	Therefore
	\ben
	...
	\geq \frac{\nu}{\sqrt{(\heps+\theta^2+1)^2+\nu^2}}\left(\frac{1}{2}\|u_2'\|^2_{L^2}  + \| \ubf \|_{L^2}^2\right).
	\een	
	Problem \eqref{vf1dcase} is then uniquely solvable thanks to Lax-Milgram theorem.
\end{proof}
%
%
%\begin{remark}
%\mrev{Not the variational problem can be proved well-posed
%with the same method for  slightly different coefficients.
%%For example for $\lambda<0$ and  Note that in the case $\lambda=0$, one can show that the problem is well-posed for all $\nu< 0$, since $a_2(\ubf,\ubf)=-\nu\|\ubf\|_{L^2}^2$ and 
...
for the case $\theta=0$. In this particular case Problem (\ref{eq:var_form2}) is well-posed for $\nu=0$. More precisely, it can be written 
as a system of two equations, the first equation (for $E_y$) being the variational formulation for the Helmholtz equation 
with the variable coefficient $k^2=(\alpha-\frac{\delta^2}{\alpha})$ \mrev{coupled to boundary conditions (\ref{eq:boundary_conditions_frequency_domain})}, and the second equation being  
$
  \left(E_x,u\right)=\left(\frac{i\delta}{\alpha}E_y,u\right)$  for all  $u\in L^{2}(\Omega)$.
The well-posedness of the former problem, with additional assumptions on boundary conditions and smoothness of the coefficients, 
was demonstrated in \cite{LMIG_thesis}. Provided the solution $E_y\in H^{1}(\Omega)$, $E_x\in L^{2}(\Omega)$ is uniquely defined from the latter equation.
\end{remark}
...
\int_{-L}^H\left( i \delta E_x+(\alpha+i\nu) E_y\right)\overline{\tilde{E}}_{y}
 ...
 \text{ for all } \tilde E_x\in L^{2}(\Omega), \; \tilde{E}_{y}\in H^{1}(\Omega).
  \end{split}
\end{align}
\urev{Let us introduce two function spaces $V_{E_x}\subset L^{2}(\Omega)$ spanned by a set of basis functions $\{\psi_{j}\}_{j=1}^{N_{1}}$ and $V_{E_{y}}\subset H^{1}(\Omega)$ spanned by $\{\phi_{i}\}_{i=1}^{N_{2}}$. }
...
\right).
\een
This expression greatly simplifies when \urev{ choosing $V_{E_x}=V_{E_{y}}$, spanned by a unique set of basis funtions $\left(\phi_{m}\right)_{m=1}^{N_{2}}$,} and
